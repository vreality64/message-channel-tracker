(() => {
    ...
    function applyI18n() {
        try {
            // Static labels
            if (titleEl)
                titleEl.textContent = chrome.i18n.getMessage("appTitle") || titleEl.textContent;
            if (prettyLabel)
                prettyLabel.textContent = chrome.i18n.getMessage("prettyJson") || prettyLabel.textContent;
            if (hint)
                hint.textContent = chrome.i18n.getMessage("prettyHint") || hint.textContent;
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        chrome.storage.sync.get({ mctEnabled: true, mctPrettyJson: false }, ({ mctEnabled, mctPrettyJson }) => {
            setUi(!!mctEnabled);
            if (prettyToggle)
                prettyToggle.checked = !!mctPrettyJson;
        });
