(() => {
  ...
  if ((window as any).__MCT_INSTALLED__) return;
  ...
  function preview(value: unknown): string {
    try {
      ...
      if ((value as any)?.constructor?.name) return (value as any).constructor.name;
  ...
  function logStyledJson(value: unknown): void {
    try {
  ...
  function logGroupCollapsedStyled(titlePairs: [string, string][]): void {
    try {
      const fmtParts: string[] = [];
      const params: string[] = [];
  ...
  (function wrapPostMessage() {
    const original = (window as any).postMessage;
    if (!original || (original as any).__MCT_WRAPPED__) return;
  ...
  (function wrapMessagePort() {
    const PortProto = (window as any).MessagePort?.prototype;
    if (!PortProto) return;
    if ((PortProto.postMessage as any)?.__MCT_WRAPPED__) return;

    const originalPostMessage = PortProto.postMessage;

    function wrappedPortPostMessage(this: MessagePort, message: unknown, transfer?: Transferable[]): void {
      if (state.enabled) {
        try {
    ...
    try {
      ...
      const listenerSet = new WeakMap<MessagePort, boolean>();
      const captureLogger = function(this: MessagePort, event: MessageEvent) {
        if (!state.enabled) return;
        try {
          const metaInfo = getPortMeta(this) || ensurePortMeta(this);
          const other = metaInfo ? (metaInfo.label === "p1" ? "p2" : "p1") : null;
      ...
      function wrappedAddEventListener(this: MessagePort, type: string, listener: EventListener, options?: boolean | AddEventListenerOptions): void {
  ...
  (function wrapMessageChannel() {
    const OriginalMC = (window as any).MessageChannel;
    if (!OriginalMC || (OriginalMC as any).__MCT_WRAPPED__) return;
    ...
    try {
      ...
      try {
        (window as any).MessageChannel = WrappedMessageChannel;
      } catch (_) {}
    }
  })();
  ...
  (function wrapBroadcastChannel() {
    ...
    if (!OriginalBC || (OriginalBC as any).__MCT_WRAPPED__) return;

    function WrappedBroadcastChannel(name: string) {
    ...
    try {
      ...
      try {
        (window as any).BroadcastChannel = WrappedBroadcastChannel;
      } catch (_) {}
    }
  })();
  ...
  (function wrapWorkers() {
    ...
    function wrapWorkerConstructor(Original: any, label: string) {
      if (!Original || (Original as any).__MCT_WRAPPED__) return Original;

      function WrappedWorker(specifier: string | URL, options?: WorkerOptions) {
    ...
    if (MaybeWrappedWorker && MaybeWrappedWorker !== OriginalWorker) {
      try {
        ...
        try {
          (window as any).Worker = MaybeWrappedWorker;
        } catch (_) {}
      }
    }
    ...
    if (MaybeWrappedSharedWorker && MaybeWrappedSharedWorker !== OriginalSharedWorker) {
      try {
        ...
        try {
          (window as any).SharedWorker = MaybeWrappedSharedWorker;
        } catch (_) {}
      }
    }
  })();
  ...
  (function wrapServiceWorkers() {
    try {
      const SWProto = (window as any).ServiceWorker?.prototype;
      if (SWProto?.postMessage && !(SWProto.postMessage as any).__MCT_WRAPPED__) {
